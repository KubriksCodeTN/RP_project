...
std::vector<coordinating::collision_t> get_collisions(const coordinating::sampling_t& path1, const coordinating::sampling_t& path2, 
    ...
    for (size_t i = 0; i < ps1.size(); ++i)
        for (size_t j = 0; j < ps2.size(); ++j)
            if (d(ps1[i], ps2[i]) <= 4 * robot_r * robot_r)
 ...
 * 
 * @note this algorithm is generalizable to n robots so this could wotk even with more than 3 shelfino
 * 
 * @todo try to find some optimizations based on known paths properties, maybe we can make some considerations on 
 * which robot to delay instead of choosing statically. DEBUG
 */
coordinating::triplet_t coordinating::coordinate(const coordinating::sampling_t& path1, const coordinating::sampling_t& path2, 
    ...
    // add time to the second and third robot incrementally to find a scheduling
    while (4){
        while(!r1_r2_ok){
